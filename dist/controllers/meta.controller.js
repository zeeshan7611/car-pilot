...
const meta_service_js_1 = require("../integrations/meta/meta.service.js");
...
const metaService = new meta_service_js_1.MetaIntegrationService();
exports.MetaController = {
    ...
    async handleCallback(req, res) {
        ...
        if (!orgId && state) {
            try {
                ...
                orgId = new mongoose_1.default.Types.ObjectId(decoded.orgId);
            }
            catch (err) {
                // ignore
            }
        ...
        const authCode = code || 'mock_auth_code';
    ...
    async getConnectedAccounts(req, res) {
        const orgId = new mongoose_1.default.Types.ObjectId(req.user.organizationId);
        const accounts = await SocialAccount_js_1.SocialAccount.find({ organizationId: orgId }).select('-accessTokenEncrypted -refreshTokenEncrypted');
    ...
    async publishReel(req, res) {
        ...
        const item = await InventoryItem_js_1.InventoryItem.findOne({ _id: inventoryId, organizationId: orgId });
        if (!item) {
